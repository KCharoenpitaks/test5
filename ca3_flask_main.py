# -*- coding: utf-8 -*-
"""
Created on Sun May  3 19:46:01 2020
...
app = Flask(__name__)
logging.basicConfig(filename='ca_flask_webserver_forresearch.log', level=logging.DEBUG)
elapsed_time_array = []
...
def main(date, lamb):
    ...
    # get nl model
    c_1 = GetTransactionData(b_0.date_prev_bd, b_0.date_today)
    c_1.create_database_connection()
    c_1.get_cursor()
    c_1.execute_fetch_commit_close()
    c_1.make_input_df()
    trade_transaction_data = c_1.df
    ...
    c_3 = MakeANNPrediction(c_2.df, b_0.date_today)
    c_3.load_the_model()
    c_3.make_nl_ann_prediction()
    nl_prediction = c_3.prediction
    ...
    c_4 = MakeNLDF(c_2.data_2, c_2.number_of_new_data, c_3.prediction, b_4.df, c_0.df)
    c_4.extract_symbol()
    ...
    return decay_result_final
    
@app.route('/ca_3/post', methods=['POST'])
def main_post():
 
    date = request.json["asof_date"]
    lamb = float(request.json["lambda"])
    
    try:
        decay_result_final = main(date, lamb)
        return decay_result_final.to_json(orient='records')
    
    except ValueError:
        raise ValueError(date+ " is not in list")
    
    except KeyError:
        raise KeyError('box_id '+date)
    
    except UnboundLocalError:
        raise UnboundLocalError(date)


@app.route('/ca_3/web', methods=['GET'])
def main_web():
    
    date = request.args.get('asof_date')
    lamb = float(request.args.get('lambda'))
    
    try:
        decay_result_final = main(date, lamb)        
        return decay_result_final.to_html()
